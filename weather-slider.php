...
for($i=1;$i<=CANT_URL;$i++)
{
	...
	if ($url != "" or $url != null) 
	{
		$content = wp_remote_fopen($url);
	 	if ($content) {
	 		$yTiempo = simplexml_load_string(str_replace('yweather:', '', $content));
	 		// Cantidad de ciudades que se van a mostrar en el template
		 	$num_mostra ++;
			...
			$html_result .= "\"><table border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td>";
			$imagen = $yTiempo->channel->item->description;
			$num_resultado = preg_match( "/<img .+?\/?>/im", $imagen, $resultado );
			if( $num_resultado > 0 )
			{
			   $html_result .= ( $resultado[ 0 ] );
			}else{
			   $html_result .= "Erro...";
			} 
		     $html_result .= "</td><td nowrap>";
			 $html_result .= '<b>'.$city.' '.$temp.'&#176;</b>';
			 $html_result .= '<br/>Min: '.$tempmin.' &#176;'.$tempunit;
			 $html_result .= '<br/>M&#225;x: '.$tempmax.' &#176;'.$tempunit;
			 $html_result .= "</td></tr></table></div>";
			 if($num_mostra%2 == 0){
			 	$html_result .= "</li>";
			 }
		 }else{
	...
	if($is_erro == 1){
		echo '<li><center>';
		echo '<table class="tbErro" border="0" cellpadding="0" cellspacing="0"><tr><td class="tdErro">&nbsp;</td>';
		echo '<td>Connection disabled</td></tr></table></center></li>';
	}else{
		echo $html_result;
	}
echo "</ul></div>";
...
function options_weather() {
	...
	echo "<div class=\"wrap\">";
	echo "<h2><img style='margin-right: 5px; width:32px; height:32px;' src='";
	echo plugins_url('weather-slider/images/weather32.png')."' alt='weather-ico'/>Weather Slider</h2>";
